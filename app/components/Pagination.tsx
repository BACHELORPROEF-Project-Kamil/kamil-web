...
export default function Pagination({ currentPage, totalPages, onPageChange }: PaginationProps) {
				...
				pages.push(
					<button
						...
						className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
							currentPage === i ? "border-2 border-accent-green font-bold" : "hover:opacity-70"
						} ${i > 3 ? "hidden md:flex" : ""}`}
					>
						{i}
					</button>,
			...
			return pagesToShow.map((p, i) =>
				typeof p === "number" ? (
					<button
						...
						className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
							currentPage === p ? "border-2 border-accent-green font-bold" : "hover:opacity-70"
						} ${i >= 3 ? "hidden md:flex" : ""}`}
					>
						{p}
					</button>
				) : (
					<span key={i} className="opacity-50 hidden md:inline">
						...
					</span>
				),
			);
	...
	return (
		<div className="flex justify-center items-center gap-2 sm:gap-4 pt-12">
			<button
			...
			<div className="bg-accent-background rounded-full px-4 sm:px-8 py-3 flex items-center gap-3 sm:gap-6 text-accent-green font-bold text-lg">
				{renderPageNumbers()}
			</div>
